...
- **`getDeviceStatus` returns 3 fields; battery, session info, and connection state are absent** ([upstream #58](https://github.com/kobiton/automate/issues/58)). The tool description documents availability, current session info, battery level, and connection state. The response returns only `deviceId`, `is_booked`, `is_online`. *Agent workaround:* use `getDeviceStatus` only to answer "is this device free right now?"; for battery level, network state, or current-session detail, the only path today is to reserve the device and probe inside the session — there is no cheaper read-path.
