...
- **`#53` asks span three architectural layers**: the MCP protocol (where SEP proposals live; multi-quarter horizon), the client implementation (skill loader, `allowed-tools` parser, sandboxed shell + FS — Claude Code and Cowork today), and tool quality (server-side gaps in `api.kobiton.com/mcp` that affect every MCP client equally).
- **The single highest-leverage near-term lever is Cowork.** As of [Cowork extensions docs](https://claude.com/docs/cowork/3p/extensions), Cowork loads the same `.claude-plugin/` plugin format as Claude Code, runs a sandboxed Ubuntu 22.04 VM on macOS, reaches user-selected local folders, and has had a plugin marketplace since 2026-02-24. A `run-automation-suite` install test on Cowork answers most of `#53` ask 2 (Claude Desktop parity / tester-first UX) without a server-side change.
...
A correct architectural answer to a `#53` ask is one of these three layers. Conflating layers (e.g., "Claude Desktop can't run the skill because of the MCP protocol") obscures what's actually fixable and on what timeline.
...
**Recommendation:** Position `run-automation-suite` as a Cowork-installable plugin (which it nearly is — same `.claude-plugin/` format). The Cowork brand surface materially reduces the "buy Claude separately" perception for tester-heavy orgs.
...
| 3 | **Claude Desktop** | ✅ via `server-filesystem` MCP | ✅ via stdio MCP | ✅ stdio + `.dxt`/`.mcpb` | ✅ HTTP + optional OAuth | ❌ not listed as a Skills surface |
| 4 | **Claude Code** (CLI/IDE) | ✅ native | ✅ native + `/sandbox` | ✅ | ✅ | ✅ |
| 5 | **Claude Cowork** | ✅ user-selected folders | ✅ Ubuntu 22.04 VM (macOS) | ✅ | ✅ | ✅ **same `.claude-plugin/` format** |
...
| F48 | [`#58`](https://github.com/kobiton/automate/issues/58) | `getDeviceStatus` | 3 fields returned vs documented 4+ (battery, session, connection-state absent) | High |
...
- Plugin-side close-out PR (fork): [`jeremylongshore/automate#54`](https://github.com/jeremylongshore/automate/pull/54)

Anthropic docs verified by direct fetch 2026-05-18:

- [Cowork extensions / 3p plugins](https://claude.com/docs/cowork/3p/extensions) — Cowork uses Claude Code's `.claude-plugin/` format
- [Use Skills in Claude](https://support.claude.com/en/articles/12512180-use-skills-in-claude) — Claude Desktop omitted from Skills surfaces
